...
class ForwardEngine:
 def __init__(self,store:ForwardStore,config:ForwardConfig=ForwardConfig(),phase7:Phase7Config=Phase7Config(),events:EventBus|None=None,run_id:str|None=None,repo:Path|None=None):
 ...
 def _write_run(self):
  ...
  self.store.append("runs",{"run_id":self.run_id,"timestamp_utc":datetime.now(UTC).isoformat(),"software_version":self.config.software_version,"git_commit":self.git_commit,"config_hash":self.config_hash,"model_version":self.config.model_version,"model_hash":hashlib.sha256(self.config.model_version.encode()).hexdigest(),"provider_modes":list(self.phase7.provider_preference),"observation_only":True},"run_id")
 def process(self,state:LiveAssetState,user_action:UserAction=UserAction.NONE)->LiveSnapshot:
  ...
  if classified.decision in (Decision.ENTER_YES,Decision.ENTER_NO) and not self.store.has_entry(state.contract_id,state.asset):
   entry={"entry_id":str(uuid.uuid4()),"observation_id":oid,"run_id":self.run_id,"asset":state.asset,"contract_id":state.contract_id,"entry_timestamp":now.isoformat(),"seconds_remaining":state.seconds_remaining,"side":classified.side,"model_probability":max(state.p_yes,1-state.p_yes),"lower_bound":state.conservative_bound,"fragility":state.fragility,"disagreement":state.disagreement,"crossing_probability":state.crossing_probability,"volatility_regime":state.volatility_regime,"reference":state.reference,"entry_underlying_price":state.current_price,"executable_ask":getattr(assessment,"ask",None),"break_even":getattr(assessment,"break_even_probability",None),"model_edge":getattr(assessment,"model_edge",None),"lcb_edge":getattr(assessment,"lcb_edge",None),"point_ev":snap.point_ev,"lcb_ev":snap.lcb_ev,"final_advisory":final.decision.value,"user_action":user_action.value,"git_commit":self.git_commit,"config_hash":self.config_hash}
