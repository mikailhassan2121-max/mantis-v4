...
class ForwardStore:
 ...
 def append(self,name,record:dict,id_field:str)->bool:
  ...
  self._ids[name].add(identifier); return True
 def has_entry(self,contract_id,asset): return any(x.get("contract_id")==contract_id and x.get("asset")==asset for x in self.read("entries"))
 def unresolved_entries(self):
  resolved={(x.get("contract_id"),x.get("asset")) for x in self.read("resolutions")}
  return [x for x in self.read("entries") if (x.get("contract_id"),x.get("asset")) not in resolved]
